...
The variables that are listed as character that should be numeric are: AGE_ENTRY, FEMALE, FIRST_GEN, GRAD_DEBT_MDN, MN_EARN_WNE_P10, PCT_ASIAN, PCT_BLACK, PCT_HISPANIC, PCT_WHITE. 

    2.  Why were they read in as type character?
  
These variables were read in as character strings because they contained entries that were not 'NA' such as 'PrivacySuppressed'. The initial command of changing 'Null' values to 'NA' did not change these character strings. 
...
The glimpse function results show the correct number of quantitative variables (31).
...
ggplot(scorecard, aes(AVGFACSAL, MN_EARN_WNE_P10))+
  ...
  ggtitle("Avg. Faculty Salary vs Avg. Earnings 10 Years Post Enrollment Across IC Levels")+
...
```
    ...
    1.  Explain why {ggplot2} chose the smoothing method.
          ...
          {ggplot2} chose the gam smoothing method because the dataset has over 1,000 observations. 
...
scorecard%>%
  filter(ICLEVEL == 3)%>%
  select(ICLEVEL, AVGFACSAL ,MN_EARN_WNE_P10)%>%
  head(, 5)
```
...
ggplot(df2,aes(x =PCT_WHITE, y = MD_EARN_WNE_P10))+
  geom_point()+
  scale_y_log10() +
...
```{r, warning=FALSE, message=FALSE}
# 6.2 Plot
ggplot(df2, aes(FEMALE, MD_EARN_WNE_P10))+
  geom_point()+
  scale_y_log10() +
  geom_smooth(method = "loess") +
